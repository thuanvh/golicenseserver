...
// The CustomerLicenseFunc type is an adapter to allow the use of ordinary
// function as CustomerLicense mutator.
type CustomerLicenseFunc func(context.Context, *ent.CustomerLicenseMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CustomerLicenseFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.CustomerLicenseMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CustomerLicenseMutation", m)
	}
	return f(ctx, mv)
}
